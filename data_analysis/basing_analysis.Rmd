...
---
# Testing hypothesis
### Test: $H_0: \mu = \mu_{\text{type}}, \quad H_1: \mu \neq \mu_{\text{type}}$
```{r}
...
```
--- 
### H0 - start time of an alarm and it’s duration are independent H1 - there is a dependence between the time of the start of an alarm and it’s duration
```{r}
...
To model the distribution of `duration_min`, we considered several candidate distributions: exponential, gamma, Weibull, and lognormal. According to all goodness-of-fit criteria (Kolmogorov–Smirnov, Cramer–von Mises, and Anderson–Darling statistics) as well as information criteria (AIC and BIC), the lognormal distribution provides the best fit to the empirical data. Therefore, in the subsequent analysis we assume:

duration_min \~ Lognormal(μ, σ²),

where the parameters μ and σ are estimated using the maximum likelihood method.
