...
$c_j$: for each of the 5 air alarm types (if the air alarm is active
then $i_j$ = 1)
...
2)  Cramer-von Mises statistic measures integrated squared distance
    ...
    $\int\_{-\infty}\^{\infty} \left( F_n(x) - F(x) \right)\^2 , dF(x)$
...
3)  Anderson-Darling statistic the same as Cramer-von Mises statistic,
    ...
    which gives more weight to tails
    $A\^2 = \int\_{-\infty}\^{\infty} \frac{ \left( F_n(x) - F(x) \right)^2 }{ F(x)\left( 1 - F(x) \right) }, dF(x)$
